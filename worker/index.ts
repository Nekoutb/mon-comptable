...
async function odooCreate(row:StoredConnection,key:string,model:string,values:Record<string,unknown>){
  const context={allowed_company_ids:[row.company_id]},origin=safeOdooOrigin(row.endpoint),headers={"content-type":"application/json; charset=utf-8","user-agent":"MyAccountant-Odoo-Connector/1.0"};
  if(row.protocol==="json2"){const response=await fetch(`${origin}/json/2/${model}/create`,{method:"POST",headers:{...headers,authorization:`bearer ${key}`,"x-odoo-database":row.database_name},body:JSON.stringify({vals_list:[values],context})});if(!response.ok)throw new Error(await odooError(response));const result=await response.json() as number|number[]|{id?:number};return Number(Array.isArray(result)?result[0]:typeof result==="object"?result.id:result)}
  if(row.protocol==="jsonrpc"){const auth=await fetch(`${origin}/jsonrpc`,{method:"POST",headers,body:JSON.stringify({jsonrpc:"2.0",method:"call",params:{service:"common",method:"authenticate",args:[row.database_name,row.login,key,{}]},id:crypto.randomUUID()})}),authenticated=await auth.json() as {result?:number};if(!authenticated.result)throw new Error("Odoo authentication failed.");const response=await fetch(`${origin}/jsonrpc`,{method:"POST",headers,body:JSON.stringify({jsonrpc:"2.0",method:"call",params:{service:"object",method:"execute_kw",args:[row.database_name,authenticated.result,key,model,"create",[values],{context}]},id:crypto.randomUUID()})}),payload=await response.json() as {result?:number;error?:{message?:string;data?:{message?:string}}};if(!payload.result)throw new Error(payload.error?.data?.message||payload.error?.message||"Odoo record creation failed.");return payload.result}
  throw new Error("Draft creation requires Odoo JSON-2 or JSON-RPC.");
}
async function odooMessage(row:StoredConnection,key:string,model:string,id:number,body:string){
  const context={allowed_company_ids:[row.company_id]},origin=safeOdooOrigin(row.endpoint),headers={"content-type":"application/json; charset=utf-8","user-agent":"MyAccountant-Odoo-Connector/1.0"};
  if(row.protocol==="json2"){const response=await fetch(`${origin}/json/2/${model}/message_post`,{method:"POST",headers:{...headers,authorization:`bearer ${key}`,"x-odoo-database":row.database_name},body:JSON.stringify({ids:[id],body,context})});if(!response.ok)throw new Error(await odooError(response));return}
  if(row.protocol==="jsonrpc"){const auth=await fetch(`${origin}/jsonrpc`,{method:"POST",headers,body:JSON.stringify({jsonrpc:"2.0",method:"call",params:{service:"common",method:"authenticate",args:[row.database_name,row.login,key,{}]},id:crypto.randomUUID()})}),authenticated=await auth.json() as {result?:number};if(!authenticated.result)throw new Error("Odoo authentication failed.");const response=await fetch(`${origin}/jsonrpc`,{method:"POST",headers,body:JSON.stringify({jsonrpc:"2.0",method:"call",params:{service:"object",method:"execute_kw",args:[row.database_name,authenticated.result,key,model,"message_post",[[id]],{body,context}]},id:crypto.randomUUID()})});if(!response.ok)throw new Error(await odooError(response));}
}
const relationName=(value:unknown)=>Array.isArray(value)?String(value[1]||value[0]||""):String(value||"");
const relationId=(value:unknown)=>Array.isArray(value)?Number(value[0]||0):Number(value||0);
...
type ApCheck={key:string;label:string;status:"passed"|"warning"|"blocked";detail:string};
type ApProposal={checks:ApCheck[];vendor:{id:number;name:string}|null;purchaseOrder:{id:number;name:string;amountTotal:number}|null;receipts:Array<{id:number;name:string;state:string}>;duplicateBillId:number|null;journal:{id:number;name:string}|null;lines:Array<{name:string;productId:number|null;quantity:number;priceUnit:number;taxIds:number[]}>;readyForDraft:boolean;generatedAt:string};
...
async function anthropicError(response:Response){const text=(await response.text()).slice(0,2000);try{const payload=JSON.parse(text) as {error?:{message?:string}};return payload.error?.message||`Anthropic returned HTTP ${response.status}.`}catch{return `Anthropic returned HTTP ${response.status}.`}}
...
async function listApDocuments(request:Request,env:Env){const owner=userId(request);if(!owner)return json({ok:false,message:"Sign in is required."},401);const rows=await env.DB.prepare("SELECT d.id,d.filename,d.content_type,d.byte_size,d.status,d.extraction_json,d.proposal_json,d.odoo_move_id,d.error_message,d.created_at,d.updated_at FROM ap_invoice_documents d WHERE d.owner_id=? AND d.tenant_id='default' AND EXISTS (SELECT 1 FROM ap_agent_runs r WHERE r.document_id=d.id AND r.owner_id=d.owner_id AND r.tenant_id=d.tenant_id AND r.model=?) ORDER BY d.created_at DESC LIMIT 50").bind(owner,AP_MODEL).all<Record<string,unknown>>();return json({ok:true,documents:(rows.results||[]).map(row=>({...row,extraction:row.extraction_json?JSON.parse(String(row.extraction_json)):null,proposal:row.proposal_json?JSON.parse(String(row.proposal_json)):null,extraction_json:undefined,proposal_json:undefined}))})}
...
function printedAmount(value:string|null){if(!value)return null;const cleaned=value.replace(/[^0-9,.-]/g,"").replace(/,(?=\d{3}(?:\D|$))/g,"").replace(",",".");const amount=Number(cleaned);return Number.isFinite(amount)?Math.abs(amount):null}
async function processApInvoice(request:Request,env:Env){
  const owner=userId(request);if(!owner)return json({ok:false,message:"Sign in is required."},401);const id=new URL(request.url).searchParams.get("id")||"";if(!id)return json({ok:false,message:"Invoice document ID is required."},400);const document=await env.DB.prepare("SELECT id,extraction_json,odoo_move_id FROM ap_invoice_documents WHERE id=? AND owner_id=? AND tenant_id='default'").bind(id,owner).first<{id:string;extraction_json:string|null;odoo_move_id:number|null}>();if(!document?.extraction_json)return json({ok:false,message:"Extract the invoice before running AP processing."},409);if(document.odoo_move_id)return json({ok:false,message:`Odoo draft ${document.odoo_move_id} already exists; AP processing is locked to prevent duplicate bills.`},409);const row=await getConnection(env,owner);if(!row)return json({ok:false,message:"Connect and save Odoo before running AP processing."},409);const key=await decrypt(row.encrypted_key,env.ERP_ENCRYPTION_KEY),extraction=JSON.parse(document.extraction_json) as ApExtraction,checks:ApCheck[]=[];
  try{
    const vendorDomain=extraction.vendor_tax_id?["|",["vat","=",extraction.vendor_tax_id],["name","ilike",extraction.vendor_name||""]]:[["name","ilike",extraction.vendor_name||""]];const vendors=await odooSearchRead(row,key,"res.partner",vendorDomain as unknown[],["name","vat","supplier_rank"],5);const vendor=vendors.length===1?{id:Number(vendors[0].id),name:String(vendors[0].name)}:null;checks.push({key:"vendor",label:"ERP vendor identification",status:vendor?"passed":"blocked",detail:vendor?`Matched ${vendor.name}.`:vendors.length>1?"Multiple possible vendors found; select the vendor in Odoo.":"No matching Odoo vendor was found."});
    const pos=extraction.purchase_order_reference?await odooSearchRead(row,key,"purchase.order",[["company_id","=",row.company_id],["name","=",extraction.purchase_order_reference]], ["name","partner_id","amount_total","state","picking_ids","order_line","currency_id"],2):[];const po=pos.length===1?pos[0]:null,poVendor=relationId(po?.partner_id);checks.push({key:"po",label:"Purchase order reference",status:po?"passed":"blocked",detail:po?`Matched ${String(po.name)}.`:"No unique purchase order was found from the extracted reference."});if(po&&vendor&&poVendor!==vendor.id)checks.push({key:"po_vendor",label:"PO vendor consistency",status:"blocked",detail:"The purchase order vendor differs from the invoice vendor."});
    const invoiceTotal=printedAmount(extraction.total_text),poTotal=po?Math.abs(Number(po.amount_total)||0):null,totalMatches=invoiceTotal!==null&&poTotal!==null&&Math.abs(invoiceTotal-poTotal)<=Math.max(.01,poTotal*.005);checks.push({key:"amount",label:"Invoice to PO amount",status:totalMatches?"passed":"blocked",detail:invoiceTotal===null||poTotal===null?"An invoice or PO total is unavailable.":`Invoice ${invoiceTotal}; PO ${poTotal}.`});
    const pickingIds=Array.isArray(po?.picking_ids)?po.picking_ids.map(Number):[],receiptRows=pickingIds.length?await odooSearchRead(row,key,"stock.picking",[["id","in",pickingIds],["company_id","=",row.company_id]],["name","state","origin"],100):[],receipts=receiptRows.map(x=>({id:Number(x.id),name:String(x.name),state:String(x.state)})),receiptsDone=receipts.length>0&&receipts.every(x=>x.state==="done");checks.push({key:"receipt",label:"Goods receipt / delivery",status:receiptsDone?"passed":"blocked",detail:receipts.length?`${receipts.filter(x=>x.state==="done").length} of ${receipts.length} Odoo receipts are done.`:"No Odoo receipt was found for the purchase order."});checks.push({key:"delivery_note",label:"Delivery note evidence",status:extraction.delivery_note_reference?"passed":"warning",detail:extraction.delivery_note_reference?`Document reference ${extraction.delivery_note_reference} was extracted.`:"No delivery note reference was visible in the invoice pack."});
    const duplicates=vendor&&extraction.invoice_number?await odooSearchRead(row,key,"account.move",[["company_id","=",row.company_id],["move_type","=","in_invoice"],["partner_id","=",vendor.id],["ref","=",extraction.invoice_number]], ["name","ref","state"],5):[],duplicateBillId=duplicates.length?Number(duplicates[0].id):null;checks.push({key:"duplicate",label:"Duplicate invoice control",status:duplicateBillId?"blocked":"passed",detail:duplicateBillId?`Odoo bill ${String(duplicates[0].name)} already uses this vendor reference.`:"No duplicate vendor reference was found in Odoo."});
    const journals=await odooSearchRead(row,key,"account.journal",[["company_id","=",row.company_id],["type","=","purchase"]],["name","code"],10),journal=journals.length?{id:Number(journals[0].id),name:String(journals[0].name)}:null;checks.push({key:"journal",label:"Purchase journal",status:journal?"passed":"blocked",detail:journal?`Draft will use ${journal.name}.`:"No Odoo purchase journal is available."});
    const poLines=po&&Array.isArray(po.order_line)?await odooSearchRead(row,key,"purchase.order.line",[["id","in",po.order_line.map(Number)]],["name","product_id","product_qty","qty_received","price_unit","taxes_id"],200):[],quantitiesReceived=poLines.length>0&&poLines.every(line=>(Number(line.qty_received)||0)>=(Number(line.product_qty)||0));checks.push({key:"quantity",label:"Ordered to received quantities",status:quantitiesReceived?"passed":"blocked",detail:poLines.length?`${poLines.filter(line=>(Number(line.qty_received)||0)>=(Number(line.product_qty)||0)).length} of ${poLines.length} PO lines are fully received.`:"No PO quantities were available."});const lines=poLines.map(line=>({name:String(line.name||"Vendor invoice line"),productId:relationId(line.product_id)||null,quantity:Number(line.product_qty)||1,priceUnit:Number(line.price_unit)||0,taxIds:Array.isArray(line.taxes_id)?line.taxes_id.map(Number):[]}));checks.push({key:"coding",label:"Expense and VAT coding",status:lines.length?"passed":"blocked",detail:lines.length?"Coding is inherited from matched Odoo purchase-order products and taxes.":"No authoritative PO lines are available for accounting coding."});
    const readyForDraft=!!vendor&&!!po&&totalMatches&&receiptsDone&&quantitiesReceived&&!duplicateBillId&&!!journal&&lines.length>0&&!checks.some(check=>check.status==="blocked"),proposal:ApProposal={checks,vendor,purchaseOrder:po?{id:Number(po.id),name:String(po.name),amountTotal:Number(po.amount_total)||0}:null,receipts,duplicateBillId,journal,lines,readyForDraft,generatedAt:new Date().toISOString()},status=readyForDraft?"ready_for_review":"exception";await env.DB.prepare("UPDATE ap_invoice_documents SET status=?,proposal_json=?,error_message=NULL,updated_at=? WHERE id=? AND owner_id=?").bind(status,JSON.stringify(proposal),proposal.generatedAt,id,owner).run();return json({ok:true,id,status,proposal,message:readyForDraft?"AP processing completed. Review the controls and create an unposted Odoo draft.":"AP processing completed with exceptions requiring human resolution."});
  }catch(error){const message=error instanceof Error?error.message:"AP processing failed.";await env.DB.prepare("UPDATE ap_invoice_documents SET status='failed',error_message=?,updated_at=? WHERE id=? AND owner_id=?").bind(message,new Date().toISOString(),id,owner).run();return json({ok:false,id,message},422)}
}
async function createApDraft(request:Request,env:Env){
  const owner=userId(request);if(!owner)return json({ok:false,message:"Sign in is required."},401);let body:{id?:string;comment?:string};try{body=await request.json()}catch{return json({ok:false,message:"Invalid request."},400)}if(!body.id)return json({ok:false,message:"Invoice document ID is required."},400);const document=await env.DB.prepare("SELECT filename,content_type,object_key,extraction_json,proposal_json,odoo_move_id FROM ap_invoice_documents WHERE id=? AND owner_id=? AND tenant_id='default'").bind(body.id,owner).first<{filename:string;content_type:string;object_key:string;extraction_json:string|null;proposal_json:string|null;odoo_move_id:number|null}>();if(!document?.extraction_json||!document.proposal_json)return json({ok:false,message:"Complete AP processing before creating a draft."},409);if(document.odoo_move_id)return json({ok:true,odooMoveId:document.odoo_move_id,message:"This invoice already has an Odoo draft."});const extraction=JSON.parse(document.extraction_json) as ApExtraction,proposal=JSON.parse(document.proposal_json) as ApProposal;if(!proposal.readyForDraft||!proposal.vendor||!proposal.purchaseOrder||!proposal.journal)return json({ok:false,message:"Resolve all blocked AP controls before creating an Odoo draft."},409);const row=await getConnection(env,owner);if(!row)return json({ok:false,message:"Connect Odoo first."},409);const key=await decrypt(row.encrypted_key,env.ERP_ENCRYPTION_KEY);
  let moveId=0;
  try{
    const invoiceLines=proposal.lines.map(line=>[0,0,{name:line.name,product_id:line.productId||false,quantity:line.quantity,price_unit:line.priceUnit,tax_ids:[[6,0,line.taxIds]]}]);moveId=await odooCreate(row,key,"account.move",{move_type:"in_invoice",company_id:row.company_id,partner_id:proposal.vendor.id,journal_id:proposal.journal.id,ref:extraction.invoice_number||undefined,invoice_date:extraction.invoice_date||undefined,invoice_date_due:extraction.due_date||undefined,invoice_origin:proposal.purchaseOrder.name,invoice_line_ids:invoiceLines});if(!moveId)throw new Error("Odoo did not return a vendor bill identifier.");const now=new Date().toISOString();await env.DB.prepare("UPDATE ap_invoice_documents SET status='odoo_draft_created_with_warning',odoo_move_id=?,updated_at=? WHERE id=? AND owner_id=?").bind(moveId,now,body.id,owner).run();
    const object=await env.DOCUMENTS.get(document.object_key);if(!object)throw new Error("The source invoice pack could not be retrieved for attachment.");const fileBytes=await object.arrayBuffer();await odooCreate(row,key,"ir.attachment",{name:document.filename,type:"binary",datas:bytesToBase64(new Uint8Array(fileBytes)),mimetype:document.content_type,res_model:"account.move",res_id:moveId});const comment=(body.comment||"").trim().slice(0,2000),auditComment=`My Accountant AP controls completed. Source document: ${document.filename}. Purchase order: ${proposal.purchaseOrder.name}. ${comment}`;await odooMessage(row,key,"account.move",moveId,auditComment);await env.DB.prepare("UPDATE ap_invoice_documents SET status='odoo_draft_created',error_message=NULL,updated_at=? WHERE id=? AND owner_id=?").bind(new Date().toISOString(),body.id,owner).run();return json({ok:true,odooMoveId:moveId,status:"odoo_draft_created",message:`Odoo vendor bill draft ${moveId} created with the invoice pack. Manual review and posting are required.`});
  }catch(error){const message=error instanceof Error?error.message:"Odoo draft creation failed.";if(moveId){await env.DB.prepare("UPDATE ap_invoice_documents SET error_message=?,updated_at=? WHERE id=? AND owner_id=?").bind(`Odoo draft ${moveId} exists, but supporting-document completion needs attention: ${message}`,new Date().toISOString(),body.id,owner).run();return json({ok:false,odooMoveId:moveId,message:`Odoo draft ${moveId} was created, but the attachment or audit comment needs manual completion. No duplicate draft will be created.`},422)}return json({ok:false,message},422)}
}
async function extractApInvoice(request:Request,env:Env){
  ...
  const bytes=await entry.arrayBuffer(),hash=await sha256Hex(bytes),existing=await env.DB.prepare("SELECT id,status,object_key FROM ap_invoice_documents WHERE owner_id=? AND tenant_id='default' AND sha256=?").bind(owner,hash).first<{id:string;status:string;object_key:string}>();if(existing&&existing.status!=="failed")return json({ok:true,duplicate:true,id:existing.id,status:existing.status,message:"This exact document is already in the review queue."});
  const id=existing?.id||crypto.randomUUID(),now=new Date().toISOString(),objectKey=existing?.object_key||`default/${owner}/${id}/${entry.name.replace(/[^a-zA-Z0-9._-]/g,"_")}`;await env.DOCUMENTS.put(objectKey,bytes,{httpMetadata:{contentType:entry.type}});if(existing)await env.DB.prepare("UPDATE ap_invoice_documents SET filename=?,content_type=?,byte_size=?,status='extracting',extraction_json=NULL,error_message=NULL,updated_at=? WHERE id=? AND owner_id=?").bind(entry.name,entry.type,entry.size,now,id,owner).run();else await env.DB.prepare("INSERT INTO ap_invoice_documents(id,owner_id,tenant_id,filename,content_type,byte_size,sha256,object_key,status,created_at,updated_at) VALUES(?,?,'default',?,?,?,?,?,'extracting',?,?)").bind(id,owner,entry.name,entry.type,entry.size,hash,objectKey,now,now).run();
...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/api/ap/invoices/process" && request.method === "POST") return processApInvoice(request,env);
    if (url.pathname === "/api/ap/invoices/draft" && request.method === "POST") return createApDraft(request,env);
